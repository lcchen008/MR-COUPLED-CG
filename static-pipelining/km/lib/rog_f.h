...
unsigned short gget_key_size(__global Reduction_Object_G *object, int bucket_index)
{
...
unsigned short gget_value_size(__global Reduction_Object_G *object, int bucket_index)
{
		...
		while(DoWork)
		{
			if(global_getlock(&(object->locks)[index]))
			{
				if((object->pairs_per_bucket)[index]==0)
				{
					...
					__global char *key_size_address = (global char *)gget_address(object, k);
