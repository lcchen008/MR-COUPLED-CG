	...
	)
{
		...
		if(local_id==0)
		{
			...
			{
				int index = block_id * NUM_BUFFERS_IN_BLOCK + i;
				bufferinfos[index].finish = 1;
			}
		}
}
	...
	)
{
	...
	{	
		...
		if(use_gc == TYPE_CPU)
		{
			...
			{
				int key_size = kget_key_size(buffer, i);
				int value_size = kget_value_size(buffer, i);
		...
		else
		{
			...
			while(finished != GPU_LOCAL_THREADS)
			{
				...
				for(; i < num; i += GPU_LOCAL_THREADS)
				{
					...
					int success = linsert_from_global(&objects[gid], key, key_size, value, value_size);
