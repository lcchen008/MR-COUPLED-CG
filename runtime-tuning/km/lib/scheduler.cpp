...
void Scheduler::start1()
{
	...
	//pthread_create(&tid[0],NULL, master, arg0); //master
	pthread_create(&tid[0],NULL, pthread_start, arg1);   //cpu
	pthread_create(&tid[1],NULL, pthread_start, arg2); //gpu
	...
	while(1)
	{
		for(int i = 0; i < num_blocks; i++)
		{
			if(workersp[i].has_task==0)    //The thread block i has consumed all the tasks
			{
					...
					{
						schedule_size = worker_tuning[i].old_task_block_size/10;
					}
...
{
	void* object = ((void **)arg)[0];

	Scheduler *scheduler = reinterpret_cast<Scheduler*>(object);

	printf("Num of offsets: %d\n", scheduler->offset_number);
	...
	Worker *workers = (Worker *)scheduler->cpuqueue.enqueueMapBuffer(scheduler->workers, CL_TRUE, CL_MAP_WRITE, 0, num_blocks*sizeof(Worker), NULL, NULL, NULL);
	...
	while(1)
	{
		...
		for(int i = 0; i < num_blocks; i++)
		{
			//printf("I am master and task_offset is :%d\n", task_offset);
			if(workers[i].has_task!=1)    //The thread block i has consumed all the tasks
			{
					workers[i].task_num = task_offset;
					workers[i].has_task = 1;
					task_offset += TASK_BLOCK_SIZE;     //increase the task offset
					if(task_offset >= scheduler->offset_number)
						break;
					//printf("I am master and scheduled a task %d to %d\n", task_offset, i);
					if(i<cpu_blocks)
							cpu++;
					else
							gpu++;
	...
	while(count != num_blocks)
	{
		for(int j = 0; j < num_blocks; j++)
		{
			if(workers[j].has_task == 0)
			{
				workers[j].has_task =-1;
				count++;
			}
		}
	}
...
void *Scheduler::pthread_start(void *arg)
{
	int* ptr_type = (int *)((void **)arg)[0];
	void* object = ((void **)arg)[1];
	...
	cl_int err;
	cl::Kernel kernel(scheduler->program, "start", &err);
	err = kernel.setArg(0, scheduler->global_offset_d);
	if (err != CL_SUCCESS) 
		printf("%s\n",print_cl_errstring(err));
	err = kernel.setArg(1, scheduler->offset_number);
...
{
	cl_int err;
	cl::Kernel gpukernel(program, "start", &err);
	
	gpukernel.setArg(0, global_offset_d);
	gpukernel.setArg(1, offset_number);
	gpukernel.setArg(2, unit_size);
	gpukernel.setArg(3, global_data_d);
	gpukernel.setArg(4, TYPE_CPU);
	gpukernel.setArg(5, workers);
	gpukernel.setArg(6, roccpu);
	gpukernel.setArg(7, roggpu);
	
	cpuqueue.enqueueNDRangeKernel(gpukernel, cl::NullRange, cl::NDRange(CPU_GLOBAL_THREADS), cl::NDRange(CPU_LOCAL_THREADS), 0, 0); 
	err = cpuqueue.flush();
	if(err != CL_SUCCESS)
    {
        	printf("%s\n",print_cl_errstring(err));
	...
	/*cl::Kernel cpukernel(program, "start", &err);
    if(err != CL_SUCCESS)
    {
        	printf("%s\n",print_cl_errstring(err));
        	return;
	...
	cpukernel.setArg(0, global_offset_d);
	cpukernel.setArg(1, offset_number);
	cpukernel.setArg(2, unit_size);
	cpukernel.setArg(3, global_data_d);
	cpukernel.setArg(4, TYPE_GPU);
	cpukernel.setArg(5, workers);
	cpukernel.setArg(6, roccpu);
	cpukernel.setArg(7, NULL);*/

	gpukernel.setArg(4, TYPE_GPU);
	//gpukernel.setArg(6, roccpu);
	//gpukernel.setArg(7, roggpu);

	gpuqueue.enqueueNDRangeKernel(gpukernel, cl::NullRange, cl::NDRange(GPU_GLOBAL_THREADS), cl::NDRange(GPU_LOCAL_THREADS), 0, 0); 
	err = gpuqueue.flush();
	if(err != CL_SUCCESS)
    {
        	printf("%s\n",print_cl_errstring(err));
