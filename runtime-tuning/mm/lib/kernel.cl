...
__kernel void mergecg(__global Reduction_Object_C *object_c, __global Reduction_Object_G *object_g)
{
	...
	/*if(global_id==0)
	{
		...
		for(; i < 100; i++)
		{
			if((object_c->buckets)[i].y!=0)
			{
				int key_size1 = cget_key_size(object_c,i);
				int value_size1 = cget_value_size(object_c, i);
				__global void *key1 = cget_key_address(object_c, i);
				__global void *value1 = cget_value_address(object_c, i);

				printf("index: %d key_cpu: %d\n", i, *(__global int *)key1);
			}
		}
		...
		for(; i < 100; i++)
		{
			if((object_g->buckets)[i].y!=0)
			{
				int key_size = gget_key_size(object_g, i);
				int value_size = gget_value_size(object_g, i);
	...
	if(device==TYPE_GPU)
	{
		...
		while(!finish)
		{
			...
			if(has_task==1)
			{
				...
				for(int i = task_index + local_id; i < task_index + block_size && i < offset_number; i += GPU_LOCAL_THREADS)
				{
