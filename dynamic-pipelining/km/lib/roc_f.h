...
unsigned short cget_key_size(__global Reduction_Object_C *object, int bucket_index)
{
...
unsigned short cget_value_size(__global Reduction_Object_C *object, int bucket_index)
{
	...
	while(!finish)
	{
		while(DoWork)
		{
			...
			if(global_getlock(&((object->locks)[index])))
			{
				...
				{
					//printf("******not zero**********\n");
					unsigned short size = cget_key_size(object, index);
