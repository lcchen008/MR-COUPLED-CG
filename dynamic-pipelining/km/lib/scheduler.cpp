		...
		unsigned int offset_unit_size)
{
	...
	/*allocating workers information arrays*/
	...
	for(int i = 0; i < num_blocks; i++)
	{
		...
		workersh[i].finish_all = 0;
	}
	
	workers = cl::Buffer(context, CL_MEM_READ_ONLY|CL_MEM_ALLOC_HOST_PTR|CL_MEM_COPY_HOST_PTR, sizeof(Worker)*num_blocks, workersh, &err);
	...
	//this->testdata = (int *)malloc(sizeof(int)*num_blocks);

	//for(int i = 0; i<num_blocks; i++)
	//	(this->testdata)[i] = 0;

	//this->test_data_d = cl::Buffer(context, CL_MEM_READ_ONLY|CL_MEM_ALLOC_HOST_PTR|CL_MEM_COPY_HOST_PTR, sizeof(int)*num_blocks, testdata, &err);
...
void *Scheduler::test1(void *arg)
{
	...
	if(device_type==reducer)
	{
		...
		err = kernel.setArg(0, scheduler->workers);
		if (err != CL_SUCCESS) 
			printf("%s\n",print_cl_errstring(err));

		err = kernel.setArg(1, scheduler->buffers);
		if (err != CL_SUCCESS) 
			printf("%s\n",print_cl_errstring(err));

		err = kernel.setArg(2, scheduler->roggpu);
		if (err != CL_SUCCESS) 
			printf("%s\n",print_cl_errstring(err));

		err = kernel.setArg(3, scheduler->roccpu);
		if (err != CL_SUCCESS) 
			printf("%s\n",print_cl_errstring(err));

		err = kernel.setArg(4, scheduler->bufferinfos);
		if (err != CL_SUCCESS) 
			printf("%s\n",print_cl_errstring(err));

		//err = kernel.setArg(5, scheduler->test_data_d);
		//if (err != CL_SUCCESS) 
		//	printf("%s\n",print_cl_errstring(err));

		err = kernel.setArg(5, use_gc);
		if (err != CL_SUCCESS) 
			printf("%s\n",print_cl_errstring(err));
	...
	{	
		...
		if (err != CL_SUCCESS) 
			printf("%s\n",print_cl_errstring(err));
		err = kernel.setArg(4, scheduler->workers);
		if (err != CL_SUCCESS) 
			printf("%s\n",print_cl_errstring(err));
		err = kernel.setArg(5, scheduler->buffers);
		if (err != CL_SUCCESS) 
			printf("%s\n",print_cl_errstring(err));
		err = kernel.setArg(6, scheduler->roggpu);
		if (err != CL_SUCCESS) 
			printf("%s\n",print_cl_errstring(err));
		err = kernel.setArg(7, scheduler->roccpu);
		if (err != CL_SUCCESS) 
			printf("%s\n",print_cl_errstring(err));
		err = kernel.setArg(8, scheduler->bufferinfos);
		if (err != CL_SUCCESS) 
			printf("%s\n",print_cl_errstring(err));
		//err = kernel.setArg(9, scheduler->test_data_d);
		//if (err != CL_SUCCESS) 
		//	printf("%s\n",print_cl_errstring(err));
		err = kernel.setArg(9, use_gc);
		if (err != CL_SUCCESS) 
			printf("%s\n",print_cl_errstring(err));
...
void Scheduler::schedule()
{
	...
	/*Kvbuffer *buffersp = (Kvbuffer *)cpuqueue.enqueueMapBuffer
		(buffers, 
		CL_TRUE, 
		CL_MAP_WRITE, 
		0, 
		sizeof(Kvbuffer)*map_blocks*NUM_BUFFERS_IN_BLOCK, 
		NULL, NULL, NULL);*/
