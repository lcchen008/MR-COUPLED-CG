...
void Scheduler::mergecg()
{
	...
	cpuqueue.enqueueNDRangeKernel(kernel, cl::NullRange, cl::NDRange(CPU_GLOBAL_THREADS), cl::NDRange(CPU_LOCAL_THREADS));
...
void Scheduler::mergegc()
{
	printf("in merge gc...\n");
	cl_int err;
	cl::Kernel kernel(program, "mergegc", &err);
	if(err != CL_SUCCESS)
    {
        	printf("%s\n",print_cl_errstring(err));
        	return;
	...
	kernel.setArg(0, roggpu);
	kernel.setArg(1, roccpu);

	gpuqueue.enqueueNDRangeKernel(kernel, cl::NullRange, cl::NDRange(GPU_GLOBAL_THREADS), cl::NDRange(GPU_LOCAL_THREADS));
