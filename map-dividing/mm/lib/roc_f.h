...
unsigned short key_size, void *value, unsigned short value_size)
{
	...
	copyVal_private_to_private(&tmp, key, key_size);
		...
		while(DoWork)
		{
			...
			if(global_getlock(&((object->locks)[index])))
			{
				...
				{	
					...
					__global void *key_data_start = cget_address(object, k);
					...
					//__global void *key1 = cget_key_address(object, index);
					//printf("key is: %d and index is: %d\n", *(__global int *)key1, index);
